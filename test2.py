...
class Compare:
    def __init__(self):
        self.delta_log = Logger('delta_log', config.delta_tables_path.format(date=datelist[1]))
        self.all_log = Logger('all_log', config.ods_tables_path.format(date=datelist[1]))
        self.his_log = Logger('his_log', config.his_tables_path.format(date=datelist[1]))
        self.read_me_log = Logger('read_me_log', config.read_me_path.format(date=datelist[1]))
        self.compare_generate_log = Logger("compare_generate_log", config.compare_generate_path.format(date=datelist[1]))
        self.job = Logger('job', config.job_schedule_path.format(date=datelist[1]))
        self.alter_table = Logger('alter_table', config.alter_table_path.format(date=datelist[1]))

    def main(self):
        if return_dict:
            return return_dict

        self.delta_log.log("connect to {} user {} using {};".format(edwdb,edwuser,edwpwd))
        self.all_log.log("connect to {} user {} using {};".format(edwdb,edwuser,edwpwd))
        self.his_log.log("connect to {} user {} using {};".format(edwdb,edwuser,edwpwd))
        self.job.log("connect to {} user {} using {};".format(dwmmdb,dwmmuser,dwmmpwd))
